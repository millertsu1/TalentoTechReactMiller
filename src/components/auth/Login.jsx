...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.error && response.error.data.status == "error") {
                ...
                const userNameUpperCase = response.data.user.name.toUpperCase();
                toast.success(`Bienvenido de  nuevo ${userNameUpperCase}`, {
                    icon: '👋🏻',
                  }).then(() => {
    ...
    return (
        <div className="max-w-lg w-full mx-auto px-5 py-[12%]">
            {!error ? null :
                (<div className="flex justify-center items-center bg-slate-100 text-red-500 font-bold">
                    Datos Invalidos
                </div>
                )}
